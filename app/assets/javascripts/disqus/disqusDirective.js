...
.directive('disqusCount', ['$window', function($window){
  return {
    restrict: 'E',
    scope: {
      post: '@'
    },
    template: '<span class="disqus-comment-count" data-disqus-identifier="{{post}}">0</span>',
    link: function(scope,element,attrs){
      
      scope.$watch('post', refreshPost, true);
...
.directive('disqusScript',[function(){
  return {
    restrict: 'M',
    link: function(scope, element, attrs){
      
      var scriptLabel = document.createElement('script');
      scriptLabel.id = 'dsq-count-scr';
      scriptLabel.type = 'text/javascript';
      scriptLabel.src = '//pitux.disqus.com/count.js';
      scriptLabel.async = true;
      document.getElementsByTagName('body')[0].appendChild(scriptLabel);
